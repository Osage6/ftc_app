...
public class MitchellTeleop extends MitchellBase {
    ...
    public void init() {
        ...
        tray_ramp = new PioneersTimeRamp(MitchellBotCalibration.TRAY_COLLECT_POSITION, MitchellBotCalibration.TRAY_COLLECT_POSITION, 1,
        ...
        tray_state = ArmState.COLLECTING;
        in_wiggle = false;
    }
    ...
    public void init_loop() {
        ...
        set_tray_angle(MitchellBotCalibration.TRAY_COLLECT_POSITION);
        open_tray();
    ...
    private void arm_servo_loop() {
        if (gamepad2.back) {
            arm_servo.setPosition(MitchellBotCalibration.arm_servo_DEPLOYED);
        } else {
            arm_servo.setPosition(MitchellBotCalibration.arm_servo_RETRACTED);
        }
    }
    ...
    protected void tray_loop() {
        switch (tray_state) {
            ...
            case DRIVING:
                if (gamepad2.dpad_up) {
                    close_tray();
                    tray_state = ArmState.TO_DEPLOY;
                } else if (gamepad2.dpad_down) {
                    close_tray();
                    tray_state = ArmState.TO_COLLECT;
                }
                break;
            ...
            case TO_DRIVE:
                if (tray_timer.seconds() > 0.5) {
                    tray_ramp.reset(tray_position(), MitchellBotCalibration.TRAY_DRIVE_POSTION, MitchellBotCalibration.TRAY_RAMP_DURATION);
                    tray_state = ArmState.DRIVING;
                }
                break;

            case TO_DEPLOY:
                tray_ramp.reset(tray_position(), MitchellBotCalibration.TRAY_DEPLOY_POSITION, MitchellBotCalibration.TRAY_RAMP_DURATION);
                tray_state = ArmState.DEPLOYED;
                break;

            case TO_COLLECT:
                tray_ramp.reset(tray_position(), MitchellBotCalibration.TRAY_COLLECT_POSITION, MitchellBotCalibration.TRAY_RAMP_DURATION);
    ...
    protected void pinch_loop() {
        if (gamepad2.y) {
            ...
            switch (wiggle_state) {
                ...
                case SHIFT_LEFT:
                    left_pinch.setPosition(MitchellBotCalibration.TRAY_PINCH_CLOSE);
                    right_pinch.setPosition(MitchellBotCalibration.TRAY_PINCH_OPEN);
                    break;

                case SHIFT_RIGHT:
                    left_pinch.setPosition(MitchellBotCalibration.TRAY_PINCH_OPEN);
                    right_pinch.setPosition(MitchellBotCalibration.TRAY_PINCH_CLOSE);
                    break;
            ...
            if (wiggle_timer.seconds() > MitchellBotCalibration.WIGGLE_WAIT) {
...
//            set_tray_pinch(Math.abs(gamepad2.right_stick_x));
    ...
    protected void elevator_loop() {
        ...
        if (gamepad2.start) {
            elevator_power = gamepad2.left_stick_x * MitchellBotCalibration.MAX_ELEVATOR_ADJUST;
            right_elevator.setPower(-elevator_power);
            left_elevator.setPower(elevator_power);
        } else if (Math.abs(elevator_power) > MitchellBotCalibration.ELEVATOR_DEADZONE) {
            right_elevator.setPower(-elevator_power);
            left_elevator.setPower(-elevator_power);
        } else {
            right_elevator.setPower(0);
    ...
    protected void collector_loop() {
        ...
        if (gamepad2.left_bumper) {
            left_intake.setPower(-MitchellBotCalibration.MAX_INTAKE_POWER * reverse);
            right_intake.setPower(MitchellBotCalibration.MAX_INTAKE_POWER * reverse);
        } else if (gamepad2.right_bumper) {
            left_intake.setPower(MitchellBotCalibration.REVERSE_POWER * reverse);
            right_intake.setPower(-MitchellBotCalibration.REVERSE_POWER * reverse);
        } else if (in_wiggle) {
            left_intake.setPower(-MitchellBotCalibration.SLOW_COLLECT);
            right_intake.setPower(MitchellBotCalibration.SLOW_COLLECT);
        } else {
            left_intake.setPower(gamepad2.left_trigger * -MitchellBotCalibration.MAX_INTAKE_POWER * reverse);
            right_intake.setPower(gamepad2.right_trigger * MitchellBotCalibration.MAX_INTAKE_POWER * reverse);
        }
    }
